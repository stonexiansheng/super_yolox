...
class Patch(nn.Module):
    ...
    def forward(self, x):
        patch_top_left  = x[...,  :320,  :320]
        patch_bot_left  = x[..., 320:640,  :320]
        patch_top_right = x[...,  :320, 320:640]
        patch_bot_right = x[..., 320:640, 320:640]
        x = torch.cat((patch_top_left, patch_bot_left, patch_top_right, patch_bot_right,), dim=1,)
        # print(x.shape)
        return self.conv(x)
...
class CSPDarknet(nn.Module):
    def __init__(self, dep_mul, wid_mul, out_features=("dark2","dark3", "dark4", "dark5"), depthwise=False, act="silu",):
        ...
        # self.stem = Patch(3, base_channels, ksize=3, act=act)
